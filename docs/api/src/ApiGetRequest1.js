...
// NOTE: Define custom react hook to fetch wells and store them in the component state
function useFetchRigWells(rigId) {

  // NOTE: Define component state
  ...
  useEffect(() => {
    fetchRigWells(rigId);
  }, [rigId]); // NOTE: A new request will be sent on every rig change in App Settings
...
function App(props) {
  ...
  // NOTE: Data is still fetching. Show loading indicator
